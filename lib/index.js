...
function parseReview(raw) {
	...
	try {
		...
		if (!isRecord$1(value) || typeof value.pass !== "boolean" || !Array.isArray(value.findings)) throw new Error("invalid review object");
...
var CaptainOrchestrator = class {
	...
	async run(options) {
		...
		let currentReview = await this.review(plan, workers, reviewRoute, options, config);
		...
		return [
			currentReview.pass ? "Captain review passed." : "Captain review stopped with findings.",
			...
			diff.patch ? `Incremental diff: ${diff.changedFiles.join(", ") || "workspace changes"}` : "Incremental diff: none"
		].join("\n");
	}
	async executeTasks(tasks, acceptance, route, options, config, budget, repairContext = "") {
	...
	async review(plan, workers, route, options, config) {
		const diff = await this.readDiff();
		const prompt = reviewPrompt(plan.acceptance, workers, diff.patch);
		return parseReview((await this.call(route, prompt, options, config.orchestration.reviewerTokenBudget)).text);
	}
	async taskText(options, route) {
		...
		return `${task}\n\nVision companion notes:\n${(await collectText(this.llm.stream({
			...request,
	...
	async changedFiles() {
		return (await this.readDiff()).changedFiles;
	}
};
